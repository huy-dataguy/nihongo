/**
 * Đồng bộ TOÀN BỘ resource JSON vào Supabase (vocab bài 1-21, grammar bài 1-20, kanji).
...
async function main() {
  ...
  // Bài 14-21 — schema đồng nhất: { school_name, contact, lesson: { title, content, vocabulary: { <cats>: [{id, japanese, vietnamese}], kanji: [{id, kanji, reading?, vietnamese}] } } }
  const filesLesson14to18: Record<number, any> = {};
  for (const lessonNum of [14, 15, 16, 17, 18, 19, 20, 21]) {
    const filePath = path.join(resourcesDir, `vocab/n5_online_vocab_lesson_${lessonNum}.json`);
    if (!fs.existsSync(filePath)) continue;
    const fileData = JSON.parse(fs.readFileSync(filePath, "utf-8"));
    filesLesson14to18[lessonNum] = fileData;
    ...
    for (const [catKey, items] of Object.entries(vocab)) {
      ...
      for (const item of items) {
        vocabRows.push({
          ...
          word: item.japanese || "",
          reading: item.reading || item.japanese || "",
          romaji: "",
          meaning: item.vietnamese || "",
  ...
  // --- KANJI from Bài 14-21 (vocabulary.kanji: [{id, kanji, reading?, vietnamese}]) ---
  for (const lessonNum of [14, 15, 16, 17, 18, 19, 20, 21]) {
  ...
  // Bài 11-13, 15-20 — cấu trúc giống bài 9: { bai_hoc, noi_dung_ngu_phap: [{ cau_truc, y_nghia, vi_du }] }
  // Bài 17, 19, 20 có thêm bang_chia_dong_tu; gắn bảng vào điểm ngữ pháp đầu tiên.
  // Bài 14 dùng cấu trúc riêng bên dưới vì bảng của bài này đã có importer riêng.
  for (const lessonNum of [11, 12, 13, 15, 16, 17, 18, 19, 20]) {
    ...
    for (const [index, item] of cauTruc.entries()) {
      grammarRows.push({
        ...
        examples: normalizeGrammarExamples(item.vi_du || []),
        summary: {},
        conjugation_tables: index === 0 ? gramData.bang_chia_dong_tu || {} : {},
        is_custom: false,
      });
    }
  }
